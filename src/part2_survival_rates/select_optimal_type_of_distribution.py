def select_optimal_type_of_distribution(optimal_parameters):
    """
     ...
     """
    optimal_parameters['distribution'] = optimal_parameters.apply(get_distribution_type, axis=1)
...
def get_distribution_type(row):
    ...
    Weibull-Gaussian model's R-squared value, it selects 'Weibull'; otherwise, it selects 'WG'.
    ...
    """
    if row['r squared (Weibull)'] + 0.025 > row['r squared (Weibull and Import-Gaussian)']:
        return 'Weibull'
    else:
        return 'WG'